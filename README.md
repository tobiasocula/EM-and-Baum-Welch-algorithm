...
We will iterate over all future timestamps, from $t=T+1$ to $t=T_{\text{end}}$, and compute the predicted states and observations from the previous values (dynamic programming). We denote $\bar{S}_t$ for the predicted state at time $t$ and $\bar{O}_t$ for the predicted observation at time $t$.  
